...
class DeployCommand extends BuildCommand {
  ...
  * githubDeploy({ targetDir, baseDir }) {
    let commmitMsg = yield runscript('git log --format=%B -n 1', { stdio: 'pipe', cwd: baseDir });
    commmitMsg = 'docs: build with -> ' + commmitMsg.stdout.toString().replace(/\n*$/, '');
    ...
    this.logger.info('publish %s from %s to gh-pages with `%s`', repo, targetDir, commmitMsg);
