...
class DocToolsBase extends Command {
  ...
  * generatePlugin({ baseDir }) {
    ...
    for (const name of Object.keys(plugins)) {
      ...
      yield checkPluginMarkdown(path.join(sourceDir, 'en/plugins', `${name}.md`));
      yield checkPluginMarkdown(path.join(sourceDir, 'zh-cn/plugins', `${name}.md`));
...
function* checkPluginMarkdown(markdown) {
