...
class DocToolsBase extends Command {
  ...
  * copyFiles({ baseDir, targetDir, docPath, external }) {
    ...
    yield this.downloadExternal(external, externalPath);
    const dirs = yield fs.readdir(externalPath);
  ...
  * downloadExternal(external, externalPath) {
    ...
    for (const repo of external) {
      ...
      yield runscript(`git clone --depth 1 ${repo} ${externalPath}/${Date.now()}`, opt);
    }
  }
}
