...
hexo.extend.helper.register('guide_toc', function() {
  ...
  for (let title in toc) {
    ...
    menu += `<dt id="title-${originTitle}" style="cursor: pointer;" class="aside-title">${title}<a id="collapse-icon-${originTitle}" class="icon closed"></a></dt><dd id=panel-${originTitle}><ul>`;
...
hexo.extend.helper.register('language_list', function() {
  ...
  if (language) {
    ...
    language.forEach(item => {
      if (lanRef[item]) {
        languageList += `<li><a id="${item}" href="#">${lanRef[item]}</a></li>`;
        languageValue.push(item);
      }
    });
    languageList += '</ul>';
    languageList += `<input id="languageInput" value="${languageValue.toString()}" style="display: none;"/>`
  }
